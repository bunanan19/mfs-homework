...
1. 当父元素设置为 `display: flex` 时，子元素的摆放方式更像 `display: block` 还是 `display: inline`？请解释为什么。
   ...
   子元素摆放方式更像display：inline，因为在flex容器中，子元素不像块状元素一样占整行，而是可以多个子元素放在一行。

2. flexbox 有哪两个轴？他们的方向可以改变吗？如果可以改变，通过设置什么属性可以改变？

   + main  axis 和cross axis，

   + 可以改变方向，flex-direction:row|row-reverse|column|column-reverse
...
3.  有哪些属性可以设置在 `container` 上的？他们分别有哪些属性？分别代表什么意思
   ...
   + flex-flow 是flex-direction`属性和`flex-wrap`属性的简写形式，默认值为`row nowrap
   + justify-content；属性定义了项目在主轴上的对齐方式。
     - `flex-start`（默认值）：左对齐
     - `flex-end`：右对齐
     - `center`： 居中
...
![image-20230916220427597](C:\Users\86153\AppData\Roaming\Typora\typora-user-images\image-20230916220427597.png)

<img src="C:\Users\86153\AppData\Roaming\Typora\typora-user-images\image-20230916220436736.png" alt="image-20230916220436736" style="zoom:80%;" />
