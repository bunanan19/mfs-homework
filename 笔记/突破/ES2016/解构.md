...
5. 下面代码声明了几个变量？值是多少？
   ...
   >这段代码声明了三个变量： `a` 、 `b` 、和 `c` 。   
   >
   >变量 `a` 的值是一个对象  `{ b: { c: "1", d: "2" } }` ，
   >
   >变量 `b` 的值是一个对象  `{ c: "1", d: "2" } }` ，
   >
   >变量 `c` 的值是字符串  `"1"` 。
...
2. 请使用解构语法实现获取斐波那契数列前10个数
   ...
   const [first, second, third, fourth, fifth, sixth, seventh, eighth, ninth, tenth] = fibonacci;
   
   console.log(first);   // 输出：0
   console.log(second);  // 输出：1
   console.log(third);   // 输出：1
   console.log(fourth);  // 输出：2
   console.log(fifth);   // 输出：3
   console.log(sixth);   // 输出：5
   console.log(seventh); // 输出：8
   console.log(eighth);  // 输出：13
   console.log(ninth);   // 输出：21
   console.log(tenth);   // 输出：34
   ```

   在上述代码中，我们将斐波那契数列的前10个数字存储在一个数组 `fibonacci` 中。然后，通过解构赋值的方式，将数组中的每个元素分配给对应的变量 `first` 、 `second` 、 `third` 、 `fourth` 、 `fifth` 、 `sixth` 、 `seventh` 、 `eighth` 、 `ninth` 和 `tenth` 。   这样，我们可以通过分别访问这些变量来获取斐波那契数列的前10个数字。输出结果将按照顺序显示每个变量的值。
