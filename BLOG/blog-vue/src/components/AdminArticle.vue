<template>
  <div>
  <md-table v-model="articles" md-card @md-selected="selected = $event" md-sort="title" md-sort-order="asc">
    <md-table-toolbar>
      <h1 class="md-title">文章管理</h1>
      <md-button class="md-primary md-raised" @click="addArticle()" >
        <md-icon>add_circle_outline</md-icon> 添加文章
      </md-button>
    </md-table-toolbar>
    ...
    <md-table-row slot="md-table-row" slot-scope="{ item }"  md-selectable="multiple" md-auto-select @sort="onSort">
      <md-table-cell md-label="标 题" md-sort-by="title" >{{ item.title }}</md-table-cell>
      <md-table-cell md-label="标 签" md-sort-by="tags">{{ item.tags.map(i=>i.name).join(',') }}</md-table-cell>
      <md-table-cell md-label="更新时间" md-sort-by="createdAt">{{ item.createdAt | datetime}}</md-table-cell>
      <md-table-cell md-label="作 者" md-sort-by="user">{{ item.user?item.user.username:'无' }}</md-table-cell>
      <md-table-cell md-label=" 编辑文章" >
...
export default {
  ...
  computed: {
    removeHit(){
      const filteredArticle = this.articles.filter(i => i.id === this.currId);  
      if (filteredArticle.length > 0) {  
        const title = filteredArticle[0].title;  
  ...
  methods: {
    ...
    //将请求封装成函数
    getData(){
      axios.get(`http://localhost:3000/api/article?offset=${this.page*this.pagination.pageSize}`
    ...
    onConfirm () {
      axios.delete(`http://localhost:3000/api/article/${this.currId}`)
      // this.active = true
      this.articles = this.articles.filter(i => i.id != this.currId)//实现数据响应
      // console.log(this.currId)
    },
    onConfirmAll () {
      Promise.all(this.selected.map(i=>axios.delete(`http://localhost:3000/api/article/${i.id}`)))
      .then((resArr)=>{
        ...
        this.articles = this.articles.filter(i => !this.selected.map(j=>j.id).includes(i.id))//实现数据响应
        this.selected = []
      })      
    },
    onCancel () {
    ...
    updatePagination (page, pageSize, sort, sortOrder) {
      ...
      // axios.get(`http://localhost:3000/api/article?offset=${this.page*this.pagination.pageSize}&pageSize=${this.pagination.pageSize}`).then(({data:res})=>{
      //   if(!res.err){
      //     this.articles = res.data
      //     this.pagination = res.pagination
      //     console.log(this.pagination)
  ...
  watch: {
    'currArticle': function(newVal) {
      if(this.currId){
        Object.assign(this.articles.filter(i=>i.id==this.currId)[0], newVal)//实现数据响应式，即删除后，列表的内容更新，
