<template>
  <div>
  <md-table v-model="users" md-card @md-selected="selected = $event" md-sort="title" md-sort-order="asc">
    <md-table-toolbar>
    ...
    <md-table-row slot="md-table-row" slot-scope="{ item }"  md-selectable="multiple" md-auto-select @sort="onSort">
      ...
      <md-table-cell md-label="名 字" md-sort-by="users">{{ item.name }}</md-table-cell>
...
export default {
  ...
  computed: {
    removeHit(){
      const filtereduser = this.users.filter(i => i.id === this.currId);  
      if (filtereduser.length > 0) {  
        const title = filtereduser[0].title;  
  ...
  methods: {
    ...
    onConfirm () {
      axios.delete(`http://localhost:3000/api/user/${this.currId}`)
      // this.active = true
      this.users = this.users.filter(i => i.id != this.currId)//实现数据响应
      // console.log(this.currId)
    },
    onConfirmAll () {
      Promise.all(this.selected.map(i=>axios.delete(`http://localhost:3000/api/user/${this.currId}`)))
      .then((resArr)=>{
        ...
        this.users = this.users.filter(i => !this.selected.map(j=>j.id).includes(i.id))//实现数据响应
        this.selected = []
      })      
    },
    onCancel () {
  ...
  watch: {
    'curruser': function(newVal) {
      if(this.currId){
        Object.assign(this.users.filter(i=>i.id==this.currId)[0], newVal)//实现数据响应式，即删除后，列表的内容更新，
        ...
        axios.get('http://localhost:3000/api/user').then(({data:res})=>{
          if(!res.err)
            this.users = res.data
          else
            console.log('err', res)
